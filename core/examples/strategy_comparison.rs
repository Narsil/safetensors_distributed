...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source_topology =
        safetensors_distributed::redistributor::load_or_create_topology(&source_dir)?;
    println!(
        "Source topology: {} files, {} ranks",
        source_topology.filenames().len(),
        source_topology.world_size()
    );

    // Create target topology (single rank for this example)
    use safetensors_distributed::topology::{SharedInfo, Tensor, Topology};
    ...
    let strategies = vec![
        (
            "ReadSerial+WriteUnordered",
            RedistributionStrategy::ReadSerialWriteUnordered,
        ),
        (
            "ReadUnordered+WriteSerial",
